...
-- Keep this mirror synchronized with supabase/migrations/000000000011_p1_7d_execution_safety.sql.
BEGIN;
...
BEGIN
    SELECT mission_id INTO v_mission_id FROM public.mission_executions WHERE id = p_execution_id;
    IF NOT FOUND THEN RETURN QUERY SELECT NULL::jsonb, false; RETURN; END IF;

    SELECT * INTO v_existing FROM public.mission_steps
    WHERE execution_id = p_execution_id AND idempotency_key = p_idempotency_key LIMIT 1;
    IF FOUND THEN RETURN QUERY SELECT to_jsonb(v_existing), false; RETURN; END IF;

    SELECT * INTO v_latest FROM public.mission_steps
    WHERE execution_id = p_execution_id AND operation_key = v_operation_key
    ORDER BY attempt_index DESC, created_at DESC LIMIT 1 FOR UPDATE;
    IF FOUND THEN
        IF v_latest.status = 'completed' THEN
            RETURN QUERY SELECT to_jsonb(v_latest), false; RETURN;
        END IF;
        IF v_latest.status = 'in_progress' AND v_latest.lease_expires_at IS NOT NULL
           AND v_latest.lease_expires_at > now() THEN
            RETURN QUERY SELECT to_jsonb(v_latest), false; RETURN;
        END IF;
        IF v_latest.status = 'in_progress' THEN
            UPDATE public.mission_steps SET status = 'failed', retry_category = 'EXECUTION', claim_token = NULL,
                result = jsonb_build_object('recovered', 'stale_claim'), completed_at = now()
            WHERE id = v_latest.id;
        END IF;
    END IF;

    INSERT INTO public.mission_steps (
        mission_id, execution_id, step_name, worker_role, status, attempt_index,
        idempotency_key, operation_key, claim_token, lease_expires_at,
        started_at, completed_at, result, retry_category, created_at
    ) VALUES (
        v_mission_id, p_execution_id, 'claimed_step', 'employee', 'in_progress', p_attempt_index,
        p_idempotency_key, v_operation_key, p_claim_token,
        now() + make_interval(secs => GREATEST(p_lease_seconds, 1)), now(), NULL,
        '{}'::jsonb, NULL, now()
